...
class ControlPlaneCutoverReadinessServiceTests(unittest.TestCase):
    def test_readiness_reports_ready_after_bundle_and_rehearsal(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            readiness_service, package_dir, _ = self._build_subject(Path(tmpdir), apply_rehearsal=False)
            summary = readiness_service.evaluate(
    ...
    def test_readiness_blocks_when_apply_rehearsal_required(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            readiness_service, package_dir, _ = self._build_subject(Path(tmpdir), apply_rehearsal=False)
            summary = readiness_service.evaluate(
                package_dir=str(package_dir),
    ...
    def test_readiness_blocks_when_runtime_validation_missing(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            readiness_service, package_dir, _ = self._build_subject(Path(tmpdir), apply_rehearsal=False)
            readiness_service.job_repository.reset_control_plane()
            summary = readiness_service.evaluate(
                package_dir=str(package_dir),
                target_database_url="postgresql://lsa:secret@db.example.com:5432/lsa_prod",
            )
            self.assertFalse(summary.ready)
            self.assertIn("runtime_validation_missing", summary.blockers)
    ...
    def _build_subject(
        ...
        *,
    ...
    ) -> tuple[ControlPlaneCutoverReadinessService, Path, Path]:
        ...
        return readiness_service, package_dir, fake_psql
